...
pub const FontChooser = extern struct {
    ...
    // Signals
    pub inline fn connectFontActivated(
        self: *Self,
        comptime T: type,
        callback: *const fn (self: *Self, fontname: [*c]const u8, data: ?*T) callconv(.C) void,
        data: ?*T,
        flags: gobject.ConnectFlags
    ) u64 {
        return c.g_signal_connect_data(self, "font-activated", @ptrCast(callback), data, null, @intFromEnum(flags));
    }

    pub inline fn connectFontActivatedSwapped(
        self: *Self,
        comptime T: type,
        callback: *const fn (data: *T, fontname: [*c]const u8) callconv(.C) void,
        data: *T,
        flags: gobject.ConnectFlags
    ) u64 {
        return c.g_signal_connect_data(self, "font-activated", @ptrCast(callback), data, null, @as(c.GConnectFlags, @intFromEnum(flags)) | c.G_CONNECT_SWAPPED);
    }
