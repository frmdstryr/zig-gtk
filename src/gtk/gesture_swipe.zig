...
pub const GestureSwipe = extern struct {
    const Self = @This();
    ...
    // Constructors
    extern fn gtk_gesture_swipe_new() ?*Self;
    ...
    extern fn gtk_gesture_swipe_get_velocity(self: *Self, velocity_x: *f64, velocity_y: *f64) bool;
    ...
    // Signals
    pub inline fn connectSwipe(
        self: *Self,
        comptime T: type,
        callback: *const fn (self: *Self, velocity_x: f64, velocity_y: f64, data: ?*T) callconv(.C) void,
        data: ?*T,
        flags: gobject.ConnectFlags
    ) u64 {
        return c.g_signal_connect_data(self, "swipe", @ptrCast(callback), data, null, @intFromEnum(flags));
    }

    pub inline fn connectSwipeSwapped(
        self: *Self,
        comptime T: type,
        callback: *const fn (data: *T, velocity_x: f64, velocity_y: f64) callconv(.C) void,
        data: *T,
        flags: gobject.ConnectFlags
    ) u64 {
        return c.g_signal_connect_data(self, "swipe", @ptrCast(callback), data, null, @as(c.GConnectFlags, @intFromEnum(flags)) | c.G_CONNECT_SWAPPED);
    }
