...
// StructInfo(RecentInfo) align(1) size(0)
...
pub const RecentInfo = extern struct {
    ...
    pub inline fn createAppInfo(self: *Self, app_name: [*c]const u8, err: ?*?*glib.Error) !?*gio.AppInfo {
        const tmp = gtk_recent_info_create_app_info(self, app_name, err);
