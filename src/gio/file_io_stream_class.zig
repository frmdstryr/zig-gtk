...
// StructInfo(FileIOStreamClass) align(8) size(368)
...
pub const FileIOStreamClass = extern struct {
    ...
    seek: *const fn (stream: *gio.FileIOStream, offset: i64, type_: glib.SeekType, cancellable: ?*gio.Cancellable) callconv(.C) bool,
    ...
    pub inline fn gType() usize {
        return c.gio_file_io_stream_class_get_type();
    }
};

test "gio.FileIOStreamClass" {
    std.testing.refAllDecls(@This());
    try std.testing.expectEqual(@as(usize, 368), @sizeOf(FileIOStreamClass));
}