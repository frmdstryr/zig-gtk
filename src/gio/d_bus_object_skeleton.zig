...
pub const DBusObjectSkeleton = extern struct {
    ...
    pub inline fn connectAuthorizeMethod(
        ...
        return c.g_signal_connect_data(self, "authorize-method", @ptrCast(callback), data, null, @intFromEnum(flags));
    }

    pub inline fn connectAuthorizeMethodSwapped(
        self: *Self,
        comptime T: type,
        callback: *const fn (data: *T, interface: gio.DBusInterfaceSkeleton, invocation: gio.DBusMethodInvocation) callconv(.C) void,
        data: *T,
        flags: gobject.ConnectFlags
    ) u64 {
        return c.g_signal_connect_data(self, "authorize-method", @ptrCast(callback), data, null, @as(c.GConnectFlags, @intFromEnum(flags)) | c.G_CONNECT_SWAPPED);
    }
    ...
    pub inline fn gType() usize {
        return c.gio_d_bus_object_skeleton_get_type();
    }
};
